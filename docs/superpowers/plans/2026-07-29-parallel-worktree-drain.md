...
> **ФИНАЛЬНОЕ РЕВЬЮ ВЕТКИ, Important 4 — ЭТОТ ТЕКСТ ВЫШЕ НЕВЕРЕН, НЕ КОПИРОВАТЬ.**
> «задача остаётся в Build» — неправда: `call_human` уводит карточку в **Your Call**, то есть
> для `--gc` дерево становится МЁРТВЫМ; держит его только незакоммиченное/незапушенное
> содержимое, а не стадия. Вывод «дерево никуда не денется» выживал случайно (дерево с
> конфликтом rebase грязное), но после `git rebase --abort` оно чистое и запушенное — и будет
> сметено, пока агент ждёт ответа. Именно отсюда ошибка уехала в SKILL.md; исправленная
> формулировка — в SKILL.md и в спеке, а этот блок оставлен как есть: план — исторический
> артефакт, а не источник правды.
