...
- **Тик оркестратора:**
  1. `vikunja-mcp workspace --gc` — СНАЧАЛА уборка: снести деревья, за которыми на доске
     ...
     только уже мёртвое). В норме `kept` ПУСТОЙ; непустой — повод прочитать `reason`, а не
     фон. `--gc` ходит ОДИН: с id задачи или с `--release` он не сочетается (откажет). Он
     же — единственная подкоманда, которая ЧИТАЕТ доску (значит требует конфиг и токен, но
     сама в трекере ничего не меняет); создание и освобождение дерева в трекер не ходят и
     токена не требуют — в сеть из них лезет только git (при создании — `git fetch origin`).
  2. Пока `wip.free > 0`: `next_task(exclude=[id задач, на которых у тебя ПРЯМО СЕЙЧАС
     живёт агент])`. **Клеймить или нет — решает `stage`, а не `resume`:**
     - `stage: "Queue"` (и `resume: false` — свежая, и `resume: true` — карточка в Queue,
       назначенная на тебя человеком или недоделанным клеймом) → **`claim` НУЖЕН**: именно
       он дозаводит задачу в Design и долечивает частичный клейм. Тулза сама это говорит в
       `note` («call claim(task_id) to finish moving it into Design») — слушайся её. Не
       заклеймишь — задача застрянет в Queue, слот твой она не займёт, а у пер-таск-агента
       `advance(to='build')` откажет: «moving to Build is only possible from Design».
     - `resume: true` в Design/Build (доработка после `review-failed`, ответ из Your Call) →
       клеймить НЕ надо, она уже твоя, и `claim` откажет: «you can only claim from Queue».
     - `review: true` → это НЕ твоя задача в работу, а предложение отревьюить чужую: слот
       она не занимает, дерево ей нужно review-шное — диспатчь ревьюера как в п.3
       (`--role review --at <sha из evidence этой задачи>`), build-агента и `task/<id>` не
       заводи.
     Дальше (для обеих задачных веток) `vikunja-mcp workspace <id>` → диспатч ФОНОВОГО
...
- **Основная сессия — тонкий оркестратор-насос, не исполнитель и не дизайнер.**
  ...
  следующая. (Это ПОСЛЕДОВАТЕЛЬНЫЙ дефолт, `wip.limit` = `null`/`1`. При
...
- **Коммит+пуш — часть перевода в Review, не отдельный шаг.** Пер-таск-агент
  ...
  - **Интеграция — это rebase + ПОВТОРНАЯ проверка + пуш, а не просто `git push`.** В
    ...
    отбило 3 раза подряд — `call_human`: задача остаётся в Build, а дерево никуда не
    денется (освобождение отказывается сносить незапушенное, см. ниже). В последовательном
    режиме, в основном чекауте, рецепт ТОТ ЖЕ минус одноразовая ветка.
