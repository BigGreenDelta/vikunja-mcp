...
- **`wip_saturated: true` — это НЕ пустая очередь.** Слоты все заняты, работа в очереди
  есть, положить её некуда. Дождись возврата любого из своих агентов и снова зови
...
- **Тик оркестратора:**
  1. `vikunja-mcp workspace --gc` — СНАЧАЛА уборка: снести деревья задач, которые на доске
     уже не живут (агент упал в прошлом тике и своё дерево не освободил). Отдаёт
     ...
     же — единственная команда, которая ЧИТАЕТ доску (значит требует конфиг и токен, но
     сама ничего в трекере не меняет); создание и освобождение дерева обходятся без сети.
...
- **Не завелось — цикл НЕ роняем.** Любой отказ `workspace` (не git-репо, нет `origin`, путь
  занят, нет прав) приходит как `{"error": ...}` и код возврата 1. Это НЕ повод встать и НЕ
  ...
  нормальный случай: он работает там, где стоит. Отказ повторяется тик за тиком — заведи
  `file_task`, чтобы это увидел человек, а не деградируй молча вечно.
...
- **Коммит+пуш — часть перевода в Review, не отдельный шаг.** Пер-таск-агент
  ...
  - **Работал в своём worktree — после `advance(to='review')` освободи его:**
    ...
    от главного чекаута). Отказ («working tree is dirty», «N commit(s) not on origin/…») —
    это ЗАЩИТА, а не ошибка: значит осталась незакоммиченная или незапушенная работа.
    Разберись, ЧТО осталось, доведи до пуша и повтори; руками (`rm -rf`,
    `git worktree remove --force`) дерево не снимай — так теряют работу. И не продолжай
    жить в дереве после `advance`: задача уже не в Build, значит для `--gc` следующего тика
    это дерево МЁРТВОЕ и он его сметёт (безопасно — только чистое и запушенное, но каталог
    из-под тебя исчезнет).
