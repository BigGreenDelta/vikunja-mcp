  ...
  1. `vikunja-mcp workspace --gc` — СНАЧАЛА уборка: снести деревья, за которыми на доске
     ...
     - **`removed_ignored` — что было УНИЧТОЖЕНО вместе с деревом, и это НЕ предупреждение
       ...
       **Действие у тебя ровно одно, и оно на будущее: вернуть уже нельзя.** Увидел поле —
       ...
       ПОСЛЕ фильтра восстановимых имён и ПОСЛЕ того, как git схлопнул игнорируемый КАТАЛОГ в одну
       запись, то есть наследует ровно ту же слепоту, что и сам ключ (как у соседнего
       ...
       «не меньше столько»; оно говорит ровно одно — сколько ЗАПИСЕЙ пережило фильтр. **И «нет этого поля» значит только «список не
       обрезали», а НЕ «сверх названного ничего не погибло»:** под обрезкой то же схлопывание
       работает по-прежнему (замерено — 2 названные записи против 32 уничтоженных файлов), так что
       односторонность чтения из предыдущего буллета обрезка не меняет ни в одну из двух сторон.
