...
- **Пер-таск-агент ведёт ВЕСЬ таск сам** (свежий на каждую задачу; старшая
  ...
  (см. «Откуда потолок»); не назовёшь — он возьмёт дефолтные 6. Дальше агент
...
- **Коммит+пуш — часть перевода в Review, не отдельный шаг.** Пер-таск-агент
  ...
  - **Интеграция — это rebase + ПОВТОРНАЯ проверка + пуш, а не просто `git push`.** В
    ...
    # ОТБИЛО? СНАЧАЛА выясни, КТО выиграл гонку, и только потом решай про следующий круг:
  ...
  - **Круг тратится ТОЛЬКО на проигранную гонку — сначала посмотри, КТО выиграл.** Отбило →
    `git fetch origin && git log --oneline HEAD..origin/main`: HEAD — твой коммит на СТАРОЙ
    базе, значит это ровно те, кто тебя обогнал. **Пусто — гонки не было вовсе** (защищённая
    ветка, нет прав на пуш, pre-receive hook, не тот remote): следующий круг проиграет ровно
    так же, а стоит он полного прогона критериев — потолок на это не тратят, `call_human`
  ...
  - **Откуда потолок и почему он `2 × wip.limit`, а не константа.** Потолок обязан быть строго
    ...
    оркестратор в брифе (он видит `wip` в каждом ответе `next_task`); не назвал — **бери 6**
    (значение при дефолтном лимите 3), но не меньше. И это верхняя граница, а не ручка для
