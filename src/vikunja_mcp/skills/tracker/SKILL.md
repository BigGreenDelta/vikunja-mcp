...
- **Одновременно — не больше `wip.limit` задач** (по умолчанию, когда `wip_limit` в
  конфиге проекта не задан, их ТРИ; `wip.limit: 1` — ровно «одна задача за раз»), но не
  останавливайся между ними — см. «Непрерывная работа (loop)». Свободных слотов нет
...
- **Тик оркестратора:**
  1. `vikunja-mcp workspace --gc` — СНАЧАЛА уборка: снести деревья, за которыми на доске
     ...
     живой, и resume-агент вернётся именно в него (см. ниже). Отдаёт ТРИ списка:
...
### Браузер (playwright): изнутри его изолировать НЕЛЬЗЯ — можно только заметить

Ожидание «запусти браузер так, чтобы не мешать другим» здесь НЕ выполнимо, и знать это
надо до того, как ты на него положишься. Факты (проверены на живой сессии):

- **Браузер ОДИН на сессию, а не на агента.** У каждого процесса `claude` ровно один
  дочерний `@playwright/mcp`, а твои сиблинги — саб-агенты ТОЙ ЖЕ сессии: значит один
  сервер, один браузер, один профиль, одна ТЕКУЩАЯ страница и один вьюпорт на всех вас.
...
- **Настоящие ручки (`--isolated`, `--user-data-dir`) — аргументы ЗАПУСКА сервера** в
  `.mcp.json`/конфиге плагина: агенту они недоступны, и даже там изолируют на
  MCP-КЛИЕНТА, а не на саб-агента, — то есть твою проблему не решают вовсе.

Поэтому правила ниже не «предотврати», а «заметь и переделай»:

- **Работай с браузером ОДНИМ непрерывным заходом**: navigate → посмотрел → снял скрин.
  Окно, в которое сиблинг уводит страницу, — это ровно паузы между твоими вызовами.
- **Не верь, что страница всё ещё твоя.** В каждом ответе печатается `Page URL:` — сверь
  его с тем, куда переходил ТЫ. Не сошлось — страницу увёл сиблинг: перейди заново и
  пересними, а вывод не делай. Тот же принцип, что и с sha: проверяешь ФАКТ, а не
...
- **Файлы браузера падают в ГЛАВНЫЙ ЧЕКАУТ, а не в твоё дерево** (cwd MCP-сервера — тот
  каталог, в котором открыта сессия). Проверено: `browser_take_screenshot(filename=
  "vmcp-554-probe.png")`, вызванный агентом из дерева задачи, записал файл в корень
  главного чекаута. Отсюда двое следствий: id задачи в `filename` (дефолтное
  `page-{timestamp}` не затрёт, а `screenshot.png` — затрёт), и в `attach_file` отдавай
  АБСОЛЮТНЫЙ путь В ГЛАВНОМ ЧЕКАУТЕ — в своём дереве этого файла нет. Сам артефакт не
  коммить: он untracked и не часть твоего диффа.
- **Перед тем как приложить скрин как evidence** (см. «Визуально проверяемый результат»)
  — сверь `Page URL` ещё раз. Это единственное место, где эта гонка портит не удобство, а
  ДОКАЗАТЕЛЬСТВО: скрин чужой страницы, приложенный к твоей карточке, ревьюер примет за
  твою работу.
- **Есть у проекта своё браузерное средство** (e2e-раннер, свой playwright/puppeteer-скрипт)
  — гоняй ЕГО из своего дерева: свой процесс = свой браузер и свой профиль, то есть
  настоящая изоляция. Общий MCP-браузер — для короткого взгляда, а не для долгой сессии
  под параллельным дренажом. Порт дев-сервера при этом выводи из id, как выше.
- **Граница правила.** Всё сказанное — про саб-агентов ОДНОЙ сессии. Если в том же
  ...
  твоя ошибка и чинится только конфигом запуска: не чини, а сообщи в отчёте.
...
- **Визуально проверяемый результат — приложи скриншот.** Если правоту
  ...
  browser-тулом — сперва сверь `Page URL` и путь к файлу по «Общие ресурсы»:
  браузер один на всю сессию, скрин может оказаться со страницы сиблинга, а
  лежит он в главном чекауте, не в твоём дереве. Кто решает,
