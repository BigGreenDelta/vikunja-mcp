...
- **Тик оркестратора:**
  1. `vikunja-mcp workspace --gc` — СНАЧАЛА уборка: снести деревья, за которыми на доске
     ...
     - **`kept` — «убрать не смог, и это НЕ штатно: посмотри».** Разбирай КАЖДУЮ запись по
       ...
       НЕ стоит в Your Call — работа, за которой никто не вернётся), `unreachable-head` у
       BUILD-дерева (прерванный `git rebase origin/main` — убитый ход обрывает его ровно так —
       оставляет дерево ЧИСТЫМ, но detached, а переигранный коммит вне всех рефов; сам такой
       тред не рассосётся и не сметётся, расчистить может только человек), `half-created`
     ...
     - **`expected` — тоже НЕ снесены** (у каждой записи `released: false`, работа на месте),
       **но это ШТАТНЫЕ состояния: не ходи в них.** Их ровно два, и оба раньше жили в `kept`,
       ...
       в дерево. ТОТ ЖЕ код у build-дерева штатным НЕ считается и уходит в `kept` (см. выше):
       обоснование «это заметки ревьюера» к build-дереву не относится.
