...
- **Каталог под временные файлы — ОДИН на СЕССИЮ, а не на агента.** Проверено на живом
  ...
  перечислением (без удаления) дважды за полчаса этой задачи: под общим скрэтчпадом СОТНИ
...
- **Ссылайся на задачу человекочитаемо.** В комментах, отчётах (worklog) и любом
  ...
  - **Завёл через `file_task` — ref УЖЕ на руках**, в `filed.ref`; отдельный
    ...
    ЦЕЛЕВОГО проекта, а не твоего: так и эхай, это имя, по которому карточку ищут
    на ЕЁ доске. У `decompose` этого пока нет: дети приезжают как `{id, title}`,
    так что ref ребёнка — только через `get_task`, и это ровно тот случай, когда
    угадывать нельзя.
...
- **«Review needs a report» на отчёте, который ты ТОЧНО написал, — это НЕ «ты забыл».**
  ...
  - `worklog — arrived as null, not as a string` при том, что ты передавал ДЛИННЫЙ
    текст. **СНАЧАЛА ПРОВЕРЬ ИМЯ ПАРАМЕТРА** — опечатка (`wroklog`) выбрасывается МОЛЧА и
    даёт ровно этот же отказ (измерено на #657 сырым JSON-RPC поверх настоящего сервера);
    из причин, которые остаются, когда ты УВЕРЕН, что текст передавал, эта единственная
    чинится тобой самим — поэтому она первая. Имя верное — значит текст до тула НЕ
    ДОЕХАЛ, и **повтор тем же вызовом — не починка**: измерено,
    ...
    в любом случае. Тул не может отличить ЧЕТЫРЕ случая: опечатка в имени, потерянный ключ,
    непереданный аргумент и ЯВНО переданный `null` приезжают одинаково — как `null` (два
    средних — вообще одна и та же форма на проводе). Поэтому он и называет СОСТОЯНИЕ, а не
    причину.
...
- **`return_task`** — внешняя блокировка (чужой сервис лежит, нет зависимости,
  ...
  **Он ОТКАЗЫВАЕТ из ДВУХ стадий — Review и Done, — а работает из пяти остальных**
  ...
  - **Из Done** (#626): карточку в Done поставил ЧЕЛОВЕК, и обратный ход из Done — тоже его,
    ...
    `claim` и `review_task` обеими ветками — отказывали. Работа в Done оказалась негодной —
...
- СВОЮ задачу в Review не трогай (чужие задачи в Review — наоборот, ревьюй,
  ...
  - **Прочитал коммент — реши, ЧТО именно приехало. `[review] NEEDS WORK` НЕ значит «дефект»:**
    ...
    - **«потеряла смысл» / внешняя блокировка** — `return_task` из Build (см. «Застрял? Выход
      ...
      одновременно — ровно то, что отказ `return_task` из Done запрещает своими словами. Отличать
