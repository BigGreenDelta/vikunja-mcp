  ...
  1. `vikunja-mcp workspace --gc` — СНАЧАЛА уборка: снести деревья, за которыми на доске
     ...
     - **`removed_ignored` — что было УНИЧТОЖЕНО вместе с деревом, и это НЕ предупреждение
       ...
       **ЧЕГО ЭТО ПОЛЕ НЕ ЛОВИТ — знай ДО, а не после.** (1) Файл ВНУТРИ игнорируемого
       ...
       измерено. (2) Настройка `status.showUntrackedFiles = no` (в любом слое конфига)
       выключает В ТОЙ ЖЕ КОМАНДЕ и `??`, и `!!` — измерено, дерево с untracked
       `REAL-WORK.txt` и игнорируемым `shot.png` вернуло пустую строку, снеслось как чистое,
       и НИ ОДИН из двух сигналов не сработал. Поэтому правило ниже не факультативно: **всё,
...
- **«Review needs a report» на отчёте, который ты ТОЧНО написал, — это НЕ «ты забыл».**
  Отказ дизъюнктивный, и с #657 он НАЗЫВАЕТ и поле, и то, КАК оно приехало. Читай именно
...
- СВОЮ задачу в Review не трогай (чужие задачи в Review — наоборот, ревьюй,
  ...
  - **Прочитал коммент — реши, ЧТО именно приехало. `[review] NEEDS WORK` НЕ значит «дефект»:**
    ...
    - **«потеряла смысл» / внешняя блокировка** — `return_task` из Build (см. «Застрял? Выход
      ...
      что и `call_human` с `decompose`. С веткой дробления они выглядят парой, но метки у них
      РАЗНЫЕ (измерены обе): тут к вердикту ДОБАВЛЯЕТСЯ `blocked` и человек видит в Backlog
      `blocked` + `review-failed`, а `decompose` вердикт СНИМАЕТ и оставляет один `epic`;
    ...
    **Queue** (#705), и она становится обычной свободной задачей: `next_task` её предложит,
    ...
    «переделывается прямо сейчас», а «была отбита и ждёт, кто возьмёт»; снимает её первый же
    `advance(to='build')` нового владельца, а `claim` — нет (измерено: `Queue[review-failed]` →
    claim → `Design[review-failed]` → advance → меток нет). Ассайненный отбой не меняется ни на
