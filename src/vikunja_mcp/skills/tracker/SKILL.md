...
- **«Review needs a report» на отчёте, который ты ТОЧНО написал, — это НЕ «ты забыл».**
  ...
  - `worklog — arrived as null, not as a string` при том, что ты передавал ДЛИННЫЙ
    ...
    Оговорка одна, и она не про твой вызов: гейт BEST-EFFORT, и если он не встал, сервер
    сказал об этом ОДНОЙ строкой в stderr ПРИ СТАРТЕ — только там опечатка снова становится
    возможной. Значит текст до тула НЕ ДОЕХАЛ, и **повтор тем же вызовом — не починка**:
    измерено,
