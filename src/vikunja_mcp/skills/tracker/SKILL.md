...
- **`return_task`** — внешняя блокировка (чужой сервис лежит, нет зависимости,
  ...
  **Он ОТКАЗЫВАЕТ из ДВУХ стадий — Review и Done, — а работает из пяти остальных**
  ...
  - **Из Done** (#626): карточку в Done поставил ЧЕЛОВЕК, и обратный ход из Done — тоже его,
    ...
    **Но НЕ читай это как «теперь из Done не уводит ничто»:** human-only Done нигде не записан
    ОДНИМ правилом — каждый тул выводит его из своей стартовой стадии, поэтому тул, который
    двигает карточку и стадию не проверяет, дырку воспроизводит. Измерено, что `decompose` —
    ровно такой и на сегодня таким остаётся (уводит родителя в Backlog с меткой `epic`);
    он отдан отдельной карточкой (#649).
