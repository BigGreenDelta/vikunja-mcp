...
- **Тик оркестратора:**
  1. `vikunja-mcp workspace --gc` — СНАЧАЛА уборка: снести деревья, за которыми на доске
     ...
     - **Пустой `kept` на ОДНОМ тике ещё не значит, что на доске всё чисто.** Осматривая
       дерево, `--gc` делает в нём `git status`, а тот переписывает индекс — и на следующем
       тике grace-окно (30 мин) считает дерево «только что тронутым» и молча пропускает его,
       не заводя записи НИ В ОДИН список. Поэтому стоячая запись, до осмотра которой `--gc`
       дошёл (`dirty`, `unpushed`, `unreachable-head`), видна не на каждом тике, а примерно
       раз в окно; записи, решённые ДО первой git-команды в дереве (`half-created`,
       `self-tree`), приходят на каждом. Ничего не теряется — на первом же тике за окном
       запись возвращается. Читай `kept` каждый тик, но «пусто» понимай как «на ЭТОМ тике
       смотреть нечего», а не как «деревьев, требующих человека, нет».
