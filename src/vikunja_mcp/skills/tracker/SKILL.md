...
- **Ревьюер, вынеся вердикт, освобождает своё дерево:**
  ...
  - **У `released: false` ЧЕТЫРЕ чтения, и твоё штатное — последнее.** `--release --role review`
    ...
    **Но «один на обе роли» верно и в обратную сторону: `dirty` роли НЕ РАЗЛИЧАЕТ.** Один
    ...
    ТВОЁ дерево не отбеливает ничто), и человек будет видеть её КАЖДЫЙ тик, не зная, чья она.
...
- СВОЮ задачу в Review не трогай (чужие задачи в Review — наоборот, ревьюй,
  ...
  - **Прочитал коммент — реши, ЧТО именно приехало. `[review] NEEDS WORK` НЕ значит «дефект»:**
    ...
    другой агентский тул (свипнуты ВСЕ агентские тулы по карточке в Review, у `advance` — обе
    формы `to=`, у `review_task` — оба вердикта: сдвинул её ровно один, `review_task(needs_work)`;
    `call_human`, `return_task` и `decompose` отказали явным гейтом по стадии), поэтому ревьюер
    вынужден оформлять этим же вердиктом всё, что должен сделать ВЛАДЕЛЕЦ из Build. Что бывает
