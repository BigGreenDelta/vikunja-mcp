...
- **Карточка ТВОЯ, а все тулы говорят «not assigned to you» — это НЕ твоя ошибка, и выход
  единственный: пересоздать карточку.** Форма измерена вживую (#885, project 10, 2026-08-06):
  `claim` отчитался успехом, `get_task` показал `stage: Design` и тебя в ассайни, а вот копия
  задачи В KANBAN-ВИДЕ приехала с ПУСТЫМ `assignees` — и судят по ней все владельческие гейты.
  Карточка при этом неработаема НИ ОДНИМ тулом (`advance`, `call_human`, `return_task`,
  `decompose` отказывают одинаково) и занимает слот WIP, а `call_human` ПО НЕЙ ЖЕ тоже
  отказывает — то есть даже спросить про неё нечем.
