...
- **«Review needs a report» на отчёте, который ты ТОЧНО написал, — это НЕ «ты забыл».**
  ...
  - `worklog — not passed at all — arrived as null` при том, что ты передавал ДЛИННЫЙ
    ...
    это единственная из трёх причин, которую ты чинишь сам, поэтому она первая. Имя верное
    — значит аргумент до тула НЕ ДОЕХАЛ, и **повтор того же вызова не поможет**: измерено,
    ...
    единого перевода). Тул не может отличить эти три случая: опечатка, потерянный ключ и
    непереданный аргумент приезжают одинаково — как `null`. Поэтому он и называет
    СОСТОЯНИЕ, а не причину.
  - **Тихих форм РОВНО ЧЕТЫРЕ** (измерено): ключ отсутствует, `null`, `""`, строка из одних
    пробельных. Всё остальное — неверный ТИП (число, список, объект) — ловится валидацией
    ГРОМКО и поимённо, до нашего гарда. И «пусто» тут значит «ноль НЕпробельных символов», а
    не ноль байт: 100 неразрывных пробелов (200 байт на проводе) отвергаются так же, как
    пустая строка.
  - **Обходной путь, который работает:** переведи карточку КОРОТКИМ `worklog`, а полный
    ...
    `advance`. Маркер `[worklog]` в тексте обязателен, и «(1/N)» тоже: ревьюер читает
    журнал по маркеру, и отчёт, разложенный без него, он просто не найдёт. Плейсхолдер
  ...
  - **Порога НЕ ЗНАЕТ никто — не гадай про него и не подгоняй под него отчёт.** На #657
    ...
    **успех вообще не ограничивает порог**, если отказ недетерминированный, — а он таков
    (три отказа подряд, потом успех). Так что ни «длиннее N всегда падает», ни «до N
    безопасно» отсюда не следует. Ветвись по слову `null` в отказе — оно про ФАКТ, а не
    про размер.
