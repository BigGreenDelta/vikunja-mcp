  ...
  1. `vikunja-mcp workspace --gc` — СНАЧАЛА уборка: снести деревья, за которыми на доске
     ...
     - **`removed_ignored` — что было УНИЧТОЖЕНО вместе с деревом, и это НЕ предупреждение
       ...
       **Действие у тебя ровно одно, и оно на будущее: вернуть уже нельзя.** Увидел поле —
       ...
       ПОСЛЕ фильтра восстановимых имён и ПОСЛЕ того, как git схлопнул в одну запись каталог, в
       который НЕ ЗАХОДИЛ (а заходит он туда, под чем индекс держит хоть один путь: игнорируемый
       `d/` с одним ОТСЛЕЖИВАЕМЫМ файлом внутри печатается по файлам — измерено), то есть
