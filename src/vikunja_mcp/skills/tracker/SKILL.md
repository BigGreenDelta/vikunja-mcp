...
- **`return_task`** — внешняя блокировка (чужой сервис лежит, нет зависимости,
  ...
  **Из Review он ОТКАЗЫВАЕТ** (гейт по стадии, #590): увести карточку, стоящую под ревью,
  на ре-триаж — это не «застрял», а вынос чужой готовой работы из пайплайна. Из остальных
  стадий он по-прежнему работает.
