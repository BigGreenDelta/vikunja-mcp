     ...
     - **`main_checkout` — тот же `--gc` подтягивает ГЛАВНЫЙ чекаут, и ключа НЕТ, когда
       ...
       * `updated: true` — подвинули; `commits` говорит, на сколько, `from`/`to` — откуда куда.
         ...
         **И ровно тут возможна ПОТЕРЯ — читай `overwritten_ignored`, если он есть.** ff молча
         ...
         уже не вернуть (две измеренные оговорки — в разборе `half-applied` ниже, и `reason` там
         называет их сам); нет ⇒ НЕ доказывает, что ничего**. **И «есть» не значит ПОЛНО — это
       ...
       * `updated: false` — не подвинули; `code` говорит, что мешает. **И тут ДВА разных кода про
         ...
         **А вот ПЕРВУЮ — «nothing half-written was found afterwards — which is what was CHECKED» —
         ...
         отслеживаемого не тронуто вовсе, коммитить и выбрасывать нечего, и чекаут догонит САМ на
         следующем свипе, как только уйдёт причина отказа (замерено: `chmod 700` на мешавший
         каталог и один свип → `updated: true`); (2) «UNCLEAR» — отслеживаемое от HEAD отличается,
         ...
         посмотреть `git status` руками. Общее у форм ровно одно: `overwritten_ignored` — те же посмертные игнорируемые
         ...
         непустыми. **И на игнорируемой форме `overwritten_ignored` ЗВЕНИТ НА КАЖДОМ ТИКЕ, пока
         стоит причина отказа** (замерено: три свипа подряд называют один и тот же путь, плюс
         ЧЕТВЁРТЫЙ раз на том свипе, который наконец доводит ff), **а потеря случилась ровно на
         ПЕРВОМ.** Повтор — не новая потеря: назови путь человеку ОДИН раз, а не четыре. Что с этим
         повтором делать в КОДЕ — открытый вопрос, отданный человеку карточкой 851.
         ...
         **Исключение ровно одно, и это `half-applied`:** «остаётся старым» тут МОЖЕТ быть неверно —
         чекаут бывает ПОЛОМАН, и тогда свип его НЕ вылечит НИКОГДА, даже когда причина ушла.
         Замерено: недописанные ОТСЛЕЖИВАЕМЫЕ пути после этого сами блокируют ff как локальные
         изменения, поэтому свипы 2 и 3 отдают `blocked`, а дерево остаётся смешанным — лечит только
         человек, закоммитив или выбросив эти пути. Скажи ему это явно, а не строчкой в общем списке.
         **КОГДА именно — читай `reason`, а не наличие ключа (VMCP-252, и круг назад тут стояло
         обратное):** «Nothing tracked differs from HEAD at all» значит, что ff доедет сам, ОДНИМ
         свипом, как только уйдёт причина; «UNCLEAR» и «could NOT be checked» значат, что этого не
         знает никто, — тогда так и скажи человеку. А игнорируемые байты человека не вернуть ни в
