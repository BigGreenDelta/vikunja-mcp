     ...
     - **`main_checkout` — тот же `--gc` подтягивает ГЛАВНЫЙ чекаут, и ключа НЕТ, когда
       ...
       * `updated: true` — подвинули; `commits` говорит, на сколько, `from`/`to` — откуда куда.
         ...
         **И ровно тут возможна ПОТЕРЯ — читай `overwritten_ignored`, если он есть.** ff молча
         ...
         уже не вернуть; нет ⇒ НЕ доказывает, что ничего**. Причин у пустоты четыре, и все живые:
         узнаваемо-восстановимые имена вроде `.venv/` отфильтрованы (как у `removed_ignored`); зонд
         best-effort и при любом сбое молча отдаёт пустой список, не роняя синк; и обход каталога
         ограничен сверху. `overwritten_ignored_truncated` к этому НЕ относится — при обрезке ключ
         как раз ЕСТЬ, а в `_truncated` лежит настоящее число. Назови такие пути человеку
         отдельной строкой — это единственный след.
