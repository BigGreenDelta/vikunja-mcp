...
- **Тик оркестратора:**
  ...
  2. Пока `wip.free > 0`: `next_task(exclude=[id задач, на которых у тебя ПРЯМО СЕЙЧАС
     живёт агент])`. **Клеймить или нет — решает `stage`, а не `resume`.** `stage` есть в
     КАЖДОМ ответе, где есть `task`, и покрывает все четыре формы:
     - `stage: "Queue"` — и свежая из очереди (`resume: false`), и назначенная лично на тебя
       ...
       именно он доводит задачу до Design и долечивает частичный клейм. Тулза сама это
       говорит в `note` («call claim(task_id) to finish moving it into Design») — слушайся её.
       Не заклеймишь — задача застрянет в Queue, слот твой она не займёт, а у пер-таск-агента
     ...
     - `stage: "Review"` (`review: true`) → это НЕ твоя задача в работу, а предложение
       отревьюить чужую: слот она не занимает, и дерево ей нужно review-шное — диспатчь
       ревьюера как в п.3 (`--role review --at <sha из evidence этой задачи>`), build-агента
       и ветку `task/<id>` не заводи.
