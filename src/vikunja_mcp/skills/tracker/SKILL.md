  ...
  1. `vikunja-mcp workspace --gc` — СНАЧАЛА уборка: снести деревья, за которыми на доске
     ...
     - **`expected` — тоже НЕ снесены** (у каждой записи `released: false`, работа на месте),
       **но это ШТАТНЫЕ состояния: не ходи в них.** Их ровно два, и оба раньше жили в `kept`,
       ...
       ЛИБО сделать коммит ДОСТИЖИМЫМ (`git branch <имя> <sha>`, а `git reset --hard HEAD~1` —
       тот же эффект) — и тогда ближайший свип снимает дерево САМ, в `released`,
       а заметки живут дальше ровно там, куда ты их отвёл: на ветке либо недостижимым объектом.
     ...
     - **Стоячая запись приходит на КАЖДОМ свипе (свип = один `--gc` за тик), пока в дереве
       ...
       которое стояло тихо, ПОКА задача была жива, попадает в список на ПЕРВОМ же свипе после её
       смерти — ждать окна не надо, оно к тому моменту уже истекло. Второй: «запись в дереве» —
       это не любая правка. Окно двигают ровно две метки, которые смотрит `--gc`: содержимое
       ВЕРХНЕГО уровня дерева (новый файл рядом с README, каталог `.pytest_cache`) и индекс
       (`git add`/`commit`/`rebase` — и ОДИН `git status --porcelain`, даже в чистом дереве).
       Правка файла в ПОДКАТАЛОГЕ — самое частое, что делает агент, — не двигает ни ту, ни
       другую, и запись про такое дерево приходит на следующем же свипе. Значит молчание окна —
       не тревога, а защита от сноса каталога из-под живого агента, но защита НЕПОЛНАЯ: агент,
       который правит только файлы в подкаталогах, для окна выглядит ушедшим. Работу это не
       теряет (дерево с незакоммиченным считается `dirty`, и его не сносят), а вот каталог
       из-под тебя убрать могут — поэтому после `advance(to='review')` не считай, что ты всё ещё
       в нём стоишь, сколько бы ты в нём ни печатал.
