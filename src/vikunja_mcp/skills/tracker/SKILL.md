...
- **Тик оркестратора:**
  1. `vikunja-mcp workspace --gc` — СНАЧАЛА уборка: снести деревья, за которыми на доске
     ...
     `{"released": [...], "kept": [...], "expected": [...]}` — и читать из них надо ОДИН.
     - **`kept` — «убрать не смог, и это НЕ штатно: посмотри».** Пусто (нормальный тик) —
       читать нечего; непусто — разбирай КАЖДУЮ запись по `reason` и `code`: `dirty`
       (незакоммиченное в мёртвом дереве), `unpushed` (незапушенные коммиты у задачи, которая
       НЕ стоит в Your Call — работа, за которой никто не вернётся), `half-created`
     ...
     - **`expected` — тоже НЕ снесены** (у каждой записи `released: false`, работа на месте),
       **но это ШТАТНЫЕ состояния: не ходи в них.** Их ровно два, и оба раньше жили в `kept`,
       ...
       на конфликте ребейза или отбитом пуше, запись будет на каждом тике, пока человек не
       ответит (это часы) — сама карточка и есть его сигнал; туда же `dirty` такой карточки.
       (2) `unreachable-head`: ревьюер закоммитил заметки ВНУТРЬ своего detached-дерева — такое
       дерево не освобождается и не сметается НИКОГДА, запись вечная; лечится не человеком, а
       правилом ревьюера — вердикт комментом в трекер, а не коммитом в дерево.
