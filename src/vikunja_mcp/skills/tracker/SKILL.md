     ...
     - **`main_checkout` — тот же `--gc` подтягивает ГЛАВНЫЙ чекаут, и ключа НЕТ, когда
       ...
       * `updated: true` — подвинули; `commits` говорит, на сколько, `from`/`to` — откуда куда.
         ...
         **И ровно тут возможна ПОТЕРЯ — читай `overwritten_ignored`, если он есть.** ff молча
         ...
         уже не вернуть; нет ⇒ НЕ доказывает, что ничего**. **И «есть» не значит ПОЛНО — это
       ...
       * `updated: false` — не подвинули; `code` говорит, что мешает. **И тут ДВА разных кода про
         ...
         промолчали. Замерено ДВА входа, на которых чекаут при этом ПОЛОМАН: уже
         полуприменённый чекаут отдаёт `blocked` на КАЖДОМ следующем свипе (свип 1 —
         ...
         `blocked`: чекаут теперь СМЕШИВАЕТ два коммита, `git status` показывает приехавшее
         содержимое как незакоммиченную работу ЧЕЛОВЕКА (то есть он может закоммитить чужое как
         своё), а лежавший там игнорируемый файл мог погибнуть. Читай ДВА ключа: `half_applied` —
         отслеживаемые пути, которые упавший ff успел записать (с `half_applied_truncated` при
         обрезке, ровно как у ключа ниже); `overwritten_ignored` — те же посмертные игнорируемые
         ...
         **Исключение ровно одно, и это `half-applied`:** «остаётся старым» тут неверно — чекаут
         остаётся ПОЛОМАННЫМ, и свип его НЕ вылечит НИКОГДА, даже когда причина ушла. Замерено:
         недописанные пути после этого сами блокируют ff как локальные изменения, поэтому свипы 2 и
         3 отдают `blocked`, а дерево остаётся смешанным — лечит только человек, закоммитив или
         выбросив эти пути. Скажи ему это явно, а не строчкой в общем списке.
