...
- **Тик оркестратора:**
  ...
  2. Пока `wip.free > 0`: `next_task(exclude=[id задач, на которых у тебя ПРЯМО СЕЙЧАС
     живёт агент])`. Свежая из Queue (`resume: false`) → `claim`; твоя активная
     (`resume: true` — доработка после `review-failed`, ответ из Your Call) → клеймить НЕ
     надо, она уже твоя (`claim` на неё и откажет); `review: true` → это ревью, слот оно не
     занимает (см. ниже). Дальше `vikunja-mcp workspace <id>` → диспатч ФОНОВОГО
...
- **Коммит+пуш — часть перевода в Review, не отдельный шаг.** Пер-таск-агент
  ...
  - **Работал в своём worktree — после `advance(to='review')` освободи его:**
    ...
    **Отказ защиты выглядит НЕ как ошибка — смотри на поле, а не на код возврата:** это код
    0 и `{"released": false, ..., "reason": "working tree is dirty (…)"}` либо
    `"reason": "N commit(s) not on origin/…"`. Код 0 тут НЕ значит «освободил» — значит
    «посмотрел и не стал». Это ЗАЩИТА, а не сбой: осталась незакоммиченная или незапушенная
    работа. Разберись, ЧТО осталось, доведи до пуша и повтори; руками (`rm -rf`,
    `git worktree remove --force`) дерево не снимай — так теряют работу.
