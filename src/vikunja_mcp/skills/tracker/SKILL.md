...
- **Цикл needs_work — и НЕ каждый его исход ведёт обратно в Review.** Обычный: задача
  ...
  В обоих случаях пушить ревьюера НЕ НА ЧТО — в Review карточки нет, — и `advance` в этой ветке
  тоже не придёт. Не жди ни того, ни другого. Что делать принимающей стороне — в «После Review».
...
- СВОЮ задачу в Review не трогай (чужие задачи в Review — наоборот, ревьюй,
  ...
  - **Прочитал коммент — реши, ЧТО именно приехало. `[review] NEEDS WORK` НЕ значит «дефект»:**
    ...
    другой агентский тул: и `call_human`, и `decompose` оттуда отказывают, поэтому ревьюер
    вынужден оформлять этим же вердиктом всё, что должен сделать ВЛАДЕЛЕЦ из Build. Три случая:
    ...
    Build тоже без ассайни — и тогда `call_human` откажет («not assigned to you — claim it
    first»), `claim` откажет тоже («you can only claim from Queue»), а `next_task` её не
    предложит вовсе. Своими силами не чинится: скажи об этом в своём итоге, чтобы человек
    вернул карточку сам.
