...
- **Свободные слоты ЗАПОЛНЯЮТСЯ; перекрытие ловится на интеграции, а не предсказывается по
  ...
  пушит (`git push origin HEAD:main`; отбило — ещё круг, до 6), конфликт разруливает сам, не
  вышло — `call_human` (см. «Коммит+пуш — часть перевода в Review»). Цена реального перекрытия
...
- **Коммит+пуш — часть перевода в Review, не отдельный шаг.** Пер-таск-агент
  ...
  - **Интеграция — это rebase + ПОВТОРНАЯ проверка + пуш, а не просто `git push`.** В
    ...
    git fetch origin && git rebase origin/main \
      ...
      && git push origin HEAD:main   # отбило (не fast-forward) — повтори блок, до 6 кругов
    ...
    **А «пуш прошёл» без последних двух команд — это вера в отсутствие сообщения об ошибке,
    ...
    контекст задачи именно у тебя; не выходит — или отбило 6 раз подряд — `call_human`.
  ...
  - **Откуда 6 кругов.** Потолок обязан быть строго больше худшего ЧИСТО МЕХАНИЧЕСКОГО прогона,
    иначе он зовёт человека на арифметику. При `wip.limit = N` каждый из N−1 сиблингов, успевший
    приземлиться за время твоей интеграции, приводит с собой ещё и свой bump: 2·(N−1) кругов,
    плюс хвостовой bump приземления, опередившего твой `fetch`. При дефолтном лимите 3 это 5 —
    отсюда **6**. Прежнее «3» было ровно длиной САМОГО ОБЫЧНОГО плохого прогона (bump соседа A →
    коммит соседа B → bump B), то есть звало человека именно тогда, когда следующий круг почти
    наверняка бы выиграл. 6 — верхняя граница, а не ручка для тюнинга: без авто-релиза соперник
    только сиблинги и потолка ты просто не достигнешь, снижать его незачем. А **достиг 6 —
    значит петля НЕ СХОДИТСЯ** (столько механика не даёт), и вопрос человеку именно про это:
    «шесть кругов подряд, вот что приезжало в главную ветку каждый раз», а не «запушьте за меня».
  ...
  - **В СООБЩЕНИИ КОММИТА не должно быть литерального ci-skip-маркера — даже в кавычках, даже
    ...
    только в сообщении коммита. И после пуша проверяй не только что пуш прошёл, а что прогон
    ВООБЩЕ ЗАВЁЛСЯ на твой sha (`gh run list` или веб) — «нет прогона» и «прогон зелёный»
