...
- **артефакты идут в cwd СЕРВЕРА** (свой ты поднимаешь из bash и корней ему не
  объявляешь, поэтому у ТВОЕГО сервера это именно cwd) — запускай его из своего дерева;
  `--output-dir` уводит ТОЛЬКО авто-именованные (`page-*.yml`, `console-*.log`), а явный
  `filename` резолвится от cwd и про `--output-dir` не знает вовсе (проверено: с
  `--output-dir`, уведённым за пределы чекаута, авто-именованные уехали туда, а снапшот с
  явным именем всё равно лёг в корень). Правило «`filename` — только с префиксом
  `.playwright-mcp/`» (см. «Общий браузер») тут то же, но каталог ТВОЙ и его, скорее
  всего, НЕТ: с `--output-dir` наружу он не создаётся вообще никогда (в том прогоне так и
  не появился), а без него его заводит первый `browser_navigate`. Поэтому до первого
  явного `filename` сделай `mkdir -p "$PWD/.playwright-mcp"` в том дереве, из которого
  поднял сервер, — иначе ENOENT. Ни те, ни другие не часть твоего диффа — не коммить;
...
- **Как именно.** Клон, `uv sync`, и `vikunja_mcp.__file__` в КАЖДОМ раунде:
  ...
  Первые три строки — ТВОИ (у аудитора нет ни твоего пути, ни твоей незакоммиченной работы),
  остальные — ЕГО. Путь клона отдай ему в брифе как рабочий каталог: сам он его не угадает.
  ...
  git clone --no-hardlinks "$TREE" "$CLONE"       # бывает НЕСКОЛЬКО (у автора и у ревьюера)
  git -C "$TREE" diff HEAD > "$SP/$ID-wip.patch"  # клон берёт ЗАКОММИЧЕННОЕ — см. ниже
  git -C "$CLONE" apply "$SP/$ID-wip.patch"       # ...а аудит нужен по РАБОЧЕМУ тексту
  ...
  rm -rf "$CLONE"                          # после сдачи: клон со своим .venv весит ~100 МБ
  ```
  ...
  Пять шагов; ни один не отменяет остальных, и каждый — замер либо прямое следствие из него:
  - **Патч обязателен, и забыть его — отдельная ловушка.** `git clone` копирует РЕПОЗИТОРИЙ, а
    не рабочий каталог: незакоммиченного в клоне НЕТ вовсе (проверено сравнением свежего клона с
    деревом). Аудитор при этом отработает штатно и вернёт находку «правила, о котором ты пишешь,
    в файле нет» — верную по тому, что он видел, и ложную по факту, то есть ровно тот класс
    ошибки, ради которого второй проход и заводят. Пара `diff HEAD` + `apply` переносит рабочее
    состояние побайтово (проверено: диффы дерева и клона совпали). Закоммитил работу локально до
    запуска аудита — патч будет пустым и шаг станет no-op, но пусть остаётся: он безвреден и он
    же страхует.
...
- СВОЮ задачу в Review не трогай (чужие задачи в Review — наоборот, ревьюй,
  ...
  - **Прочитал коммент — реши, ЧТО именно приехало. `[review] NEEDS WORK` НЕ значит «дефект»:**
    ...
    Оговорка на редкий край, измеренный на живом `Workflow`: всё это работает, пока карточка
    ЗА ТОБОЙ. У карточки, которую человек положил в Review БЕЗ ассайни, отбой оставляет её в
    Build тоже без ассайни — и тогда отказывают ВСЕ маршруты выше, одним и тем же «not assigned
    to you — claim it first»: и `advance`, и `call_human`, и `return_task`, и `decompose`
    (измерено поштучно — не пробуй следующий из списка, когда откажет первый). Совет из этого
    отказа невыполним: `claim` откажет тоже («you can only claim from Queue»), а `next_task`
    карточку не предложит вовсе. Своими силами не чинится: скажи об этом в своём итоге, чтобы
    человек вернул карточку сам.
